...
#    a. 创建一个 LLMTranslatorAgent 实例，传入模型名、源设备名和目标设备名。
#       例如: llm_translator = LLMTranslatorAgent(model="qwen-max-latest", 
#                                                src_device="Nokia SR OS router", 
#                                                tgt_device="Huawei NE40E router")
#    b. 使用 [purify_configuration](file:///Users/tommyxu/Documents/configcompare/INTA-project/utils.py#L538-L570) 函数清理源配置（移除注释、多余空格等）。
#    c. 调用 `llm_translator.llm(src_config)` 方法，将净化后的源配置发送给 LLM，
#       并获取原始翻译结果。
#    d. 再次使用 [purify_configuration](file:///Users/tommyxu/Documents/configcompare/INTA-project/utils.py#L538-L570) 清理 LLM 返回的翻译结果。
#    e. 累加此次调用消耗的 Prompt Token 和 Completion Token 到全局变量 
#       [total_prompt_tokens](file:///Users/tommyxu/Documents/configcompare/INTA-project/run.py#L1050-L1050) 和 [total_completion_tokens](file:///Users/tommyxu/Documents/configcompare/INTA-project/run.py#L1051-L1051) 中，用于最终统计。
#    f. 记录日志，包括净化后的翻译结果和本次调用的 token 使用情况。
#    g. 返回净化后的翻译结果。
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--src-config-path", type=str, default="./datasets/test/nokia/configtrans_1.txt", help="Source config path")
    ...
    # ========== 功能块 2: 根据参数校验和设置全局变量 ==========python run.py --src-vendor nokia-sr --tgt-vendor huawei-ne40e --method LLM --base-model qwen-max-latest --src-config-path test_dataset/Nokia2Huawei/Nokia-with-manual/python run.py --src-vendor nokia-sr --tgt-vendor huawei-ne40e --method LLM --base-model qwen-max-latest --src-config-path test_dataset/Nokia2Huawei/Nokia-with-manual/python run.py --src-vendor nokia-sr --tgt-vendor huawei-ne40e --method LLM --base-model qwen-max-latest --src-config-path test_dataset/Nokia2Huawei/Nokia-with-manual/python run.py --src-vendor nokia-sr --tgt-vendor huawei-ne40e --method LLM --base-model qwen-max-latest --src-config-path test_dataset/Nokia2Huawei/Nokia-with-manual/python run.py --src-vendor nokia-sr --tgt-vendor huawei-ne40e --method LLM --base-model qwen-max-latest --src-config-path test_dataset/Nokia2Huawei/Nokia-with-manual/python run.py --src-vendor nokia-sr --tgt-vendor huawei-ne40e --method LLM --base-model qwen-max-latest --src-config-path test_dataset/Nokia2Huawei/Nokia-with-manual/
    ...
    for file_to_translate in files_to_translate:
        ...
        try:    # 避免小模型实验中遇到各种bug
            ...
            # ------ 子功能块 7.3: 执行翻译 (LLM/VOTE 方法) ------
            # LLM Translation
            if args.method == "LLM" or args.method == "VOTE":
                translation_llm = pure_llm(src_config, model=args.base_model)    #    - 它接收源配置 (src_config) 和要使用的 LLM 模型名称 (args.base_model)。
            ...
            # ------ 子功能块 7.4: 执行翻译和后续步骤 (INTA/VOTE 方法) ------
            # 1. 条件判断: 检查是否需要执行 INTA 流程
            #    - 如果用户指定的方法是 "INTA"，则必须执行此步骤。
            #    - 如果用户指定的方法是 "VOTE"，也需要执行 INTA 流程来获得一个候选翻译结果，
            #      以便后续与 LLM 方法的结果进行比较和投票选择。
            if args.method == "INTA" or args.method == "VOTE":
                # --- 子子功能块 7.4.1: 构建源配置树 (如果启用视图) ---
                # 2. 初始化变量
                #    - src_config_root: 用于存储源设备配置的结构化树根节点。
                #    - src_manual_content_list: 用于存储源设备配置相关手册的内容列表。
                #    初始值都设为 None。                
                src_config_root, src_manual_content_list = None, None

                # 3. (可选) 构建源配置树
                #    - 条件: 用户通过 --use-view 参数启用了视图（结构）分析功能。
                if args.use_view:
                    logger.info(f"##### Building {DEVICE_NAME_DICT[args.src_vendor]} Config Tree #####")
                    ##### Build the source Config Tree #####    
                    # a. 调用 
                    #  函数
                    #    - 输入: 清洗过的源配置 (purify_configuration(src_config, keep_manuals=True)),
                    #            预先加载的源设备命令层次结构根节点 (src_hierarchy_root),
                    #            设备类型 (args.src_vendor)。
                    #    - 目的: 将文本形式的源配置解析成一个结构化的树形对象 (src_config_root)，
                    #            方便后续查找命令节点和关联手册。
                    #    - build_config_tree 内部会根据 vendor 调用具体的构建函数，如 build_config_tree_nokia。
                    ...
                    # c. (可选) 如果提供了带命令的源配置路径 (--src-config-path-cmd)
                    if src_config_cmd:
                        #    同样为这个命令配置构建一个结构化树 (src_config_cmd_root)。
                    ...
                    # d. 获取源配置树中所有相关节点的手册内容列表
                    #    - 调用 src_config_root.get_subtree_manual_content_list() 方法。
                    #    - 这些手册内容将在后续的配置分片步骤中提供给 LLM 作为上下文，
                    #      帮助它理解如何将配置分片。
                ...
                # =======================================INTA框架开始=======================================
                # 4. 获取配置分片和意图
                    #    这是 INTA 流程的第一步：将源配置分解为更小的、具有明确功能的片段。
                    #    - 条件: 用户没有使用外部提供的分片文件 (--use-ext-config-division 为 False)。
                ...
                if args.use_ext_config_division:
                    ...
                    # d. (可选) 如果启用了视图分析，则为每个分片找到对应的源配置树节点和手册。
                    #    这会填充 config_division 中每个 item 的 "Nodes" 和 "TargetConfigManuals"/"TargetCmdManuals" 字段。
                    if args.use_view:
                        config_division = get_nodes_and_manuals(config_division, src_config_root, 'config')
                else:
                    # 如果不使用外部分片，则调用 get_division_and_intent 函数让 LLM 自动生成分片。
                    # a. 调用核心函数 get_division_and_intent。
                    #    - 输入: 源配置 (src_config), 源配置树根节点 (src_config_root, 可能为 None),
                    #            源手册内容列表 (src_manual_content_list, 可能为空),
                    #            使用的 LLM 模型 (args.base_model)。
                    #    - 目的: 利用 LLM 将源配置切分成片段，每个片段包含 "Fragment"(配置原文)、
                    #            "Function"(功能描述)、"Details"(详细参数) 等信息。
                    #    - get_division_and_intent 内部:
                    #        i.   使用 ConfigDivideAgent 调用 LLM 生成分片。
                    #        ii.  解析 LLM 返回的 JSON 格式结果。
                    #        iii. 验证返回的 JSON 是否符合预定义的 schema。
                    #        iv.  (可选) 如果有 src_config_root，则调用 get_nodes_and_manuals 为每个分片
                    #             关联源配置树节点和手册。
                    config_division = get_division_and_intent(src_config, src_config_root, src_manual_content_list, model=args.base_model)
                # b. 检查分片是否成功生成，失败则记录日志并跳过当前文件。
                if not config_division:
                    logger.info("Error: No config division found.")
                    continue
                
                # 5. (可选) 为命令配置分片关联节点
                #    - 条件: 提供了命令配置路径 (--src-config-path-cmd) 并且启用了视图分析 (--use-view)。
                #    - 目的: 如果存在单独的命令配置文件，也为其分片找到对应的源配置树节点（可能是命令级别的）。
                # 输出 (Output): 修改后的、内容更丰富的同一个 config_division 列表（原地修改并返回）。
                ...
                # 8. (可选) 执行手册检索
                #    - 条件: 用户没有使用基线手册 (--use-baseline 为 False)。
                if not args.use_baseline:
                    # a. 检查是否使用外部检索结果
                    ##### Retrieve Config Manuals #####
                    if not args.use_ext_config_retrieval:
                        # 如果不使用外部检索结果，则执行标准的配置手册检索流程。
                        # 调用 retrieve_manuals 函数。
                        # - 输入: 配置分片 (config_division), 源配置 (src_config),
                        #         目标配置手册路径 (config_manual_path), 各种检索参数 (如 top_k, 模型等)。
                        # - manual_type='config' 表示这次检索的是配置级别 (Configuration Guide) 的手册。
                        # - 目的: 为 config_division 中的每个分片，找到最相关的配置手册，并更新到
                        #         item["RetrievedConfigManuals"] 和 item["TargetConfigManuals"] 字段中。
                        ...
                        # 一般不走。。。。。。。。。。。。
                        # 如果使用外部检索结果，则跳过检索步骤并记录日志。
                        logger.info(f"Using external config retrieval...")
                    # b. (可选) 检索命令手册
                    #    - 条件: 用户通过 --use-cmd-manuals 参数启用了命令手册检索。
                    if args.use_cmd_manuals:
                        # 调用 retrieve_manuals 函数，这次 manual_type='cmd'。
                        # 找最相关的命令手册
                        # - 目的: 为 config_division 中的每个分片，找到最相关的命令手册 (Command Reference)，
                        #         并更新到 item["RetrievedCmdManuals"] 和 item["TargetCmdManuals"] 字段中。
                        ...
                        # c. (可选) 执行交叉检索 (Cross-retrieval)
                        #    - 调用 cross_retrieval_cmd 函数。
                        #    - 目的: 利用已检索到的配置手册与命令手册之间的映射关系，
                        #            进一步丰富或优化命令手册的检索结果。
                        ##### Cross-retrieval #####
                        # for cmd
                ...
                # 9. (可选) 检查是否只执行检索代理 (RA-Only)
                #    - 条件: 用户通过 --ra-only 参数启用，并且存在对应的分片文件。
                #    - 目的: 如果只想做检索实验，可以在完成检索后直接跳过后续翻译步骤。
                if args.ra_only:
                    continue
                # exit()

                # 10. 开始基于检索的翻译阶段 (RAG Translation)
                ##### translate fragments with syntax/view verification #####
                translation_time = time.time()
                # a. 调用 rag_translation 函数。
                #    - 输入: 配置分片 (config_division), 各种参数 (如 top_k, 是否使用视图验证等)。
                #    - 目的: 遍历每个配置分片，利用之前检索到的相关手册作为上下文，
                #            调用 LLM (LLMTranslatorAgent.rag 方法) 进行翻译。
                #    - rag_translation 内部:
                #        i.   遍历 config_division。
                #        ii.  为每个分片 item 收集源手册、目标配置手册、目标命令手册。
                #        iii. 调用 rag_translator.rag(...) 方法，传入手册和当前翻译上下文，生成翻译。
                #        iv.  (可选) 如果启用了视图或语法验证 (--use-view, --no-sLoop 控制)，
                #             会对生成的翻译进行检查，并调用 LLM 进行修正 (sLoop)。
                #        v.   累积生成完整的翻译结果 curr_translation。
                ...
                # 12. (可选) 执行语义验证和精炼 (lLoop)
                #     - 条件: 用户没有禁用 lLoop (--no-lLoop 为 False) 并且启用了视图分析 (--use-view)。
                ...
                if not args.no_lLoop:
                    ...
                    if args.use_view:
                        ...
                        # a. 调用 semantic_refinement_with_view 函数。
                        #    - 输入: 源配置、初步翻译、源/目标配置树、分片等。
                        #    - 目的: 进行更高层次的语义检查。
                        #    - semantic_refinement_with_view 内部:
                        #        i.   构建初步翻译的目标配置树。
                        #        ii.  调用 verify_rag 进行语义错误检测和修正。
                        #        iii. verify_rag 内部:
                        #            1. 使用 CriticAgent 调用 LLM 分析翻译中的潜在语义问题。
                        #            2. 根据分析结果，再次调用 LLMTranslatorAgent 进行针对性修正 (refine_with_semantic_rag)。
                        #            3. 验证修正后的结果是否有改进。
                        #        iv.  如果有改进，则返回精炼后的翻译 translation_inta。
                        ...
                        # 如果没有启用视图，则理论上可以进行纯语法层面的语义检查（但代码中此处是 pass）。
                        # 语义检查后进行语法检查
                        pass
