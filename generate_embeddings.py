...
# 在导入其他库之前添加 MPS 兼容性修复
import torch

# 为兼容性添加 torch.mps（如果不存在）
if not hasattr(torch, "mps"):
    # 创建一个兼容的 MPS 模块
    class CompatibleMPS:
        @staticmethod
        def is_available():
            # 检查 torch.backends.mps 是否可用
        ...
        @staticmethod
        def device_count():
            # 如果 torch.mps 存在，返回 1，否则返回 0
            if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                return 1  # MPS 只有一个设备
            return 0
    
    torch.mps = CompatibleMPS()

# 强制禁用 MPS，避免 FlagEmbedding 访问不可用后端
if hasattr(torch.backends, 'mps'):
...
model_paths = {
    ...
    'bge-m3': os.path.join(BASE_DIR, 'models', 'bge-m3'),
...
class EmbeddingModel():
    def __init__(self, model_name, corpus_type='cmd'):
        ...
        raw_path = model_paths.get(model_name, model_name)
        self.model_path = os.path.expanduser(raw_path)
        self.device = 'cpu'  # 强制使用 CPU，避免 mps/cuda 不可用导致报错
        ...
        if model_name == 'sbert':
            ...
            self.model = BGEM3FlagModel(self.model_path, use_fp16=False, device=self.device)
            # self.model = SentenceTransformer(self.model_path)
            # self.model.to(self.device)
